...
def plotly_color_to_hex(color: str) -> str:
    """Convert Plotly color strings into #RRGGBB for st.color_picker."""
    if isinstance(color, str) and re.fullmatch(r"#[0-9a-fA-F]{6}", color.strip()):
        return color.strip()

    rgb_match = re.fullmatch(r"rgb\((\d+)\s*,\s*(\d+)\s*,\s*(\d+)\)", str(color).strip())
    if rgb_match:
...
def build_color_map(df: pd.DataFrame, color_col: str) -> Dict[str, str]:
    ...
    raw_palette = px.colors.qualitative.Set2 + px.colors.qualitative.Plotly + px.colors.qualitative.Safe
    default_palette = [plotly_color_to_hex(c) for c in raw_palette]
    color_map = {}
    ...
    with st.sidebar.expander("Choose category colors", expanded=True):
        ...
        for idx, value in enumerate(values):
            ...
            color_map[value] = st.color_picker(
                label,
                value=default_color,
                key=safe_widget_key(color_col, value),
            )
